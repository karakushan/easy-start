...
//Иннициализируем классы
	...
	function es_admin_menu(){
		...
		if ($config->data["show_admin_menu"]===true) {
			add_menu_page('Easy Start', 'Easy Start', 'manage_options', 'easy-start',array( $this, 'es_menu_page' ),'dashicons-schedule',61);
		}
		...
		add_submenu_page( 'easy-start',__( 'Export', 'easy-start' ),__( 'Export', 'easy-start' ), 'manage_options', 'easy-start-import-export',array($this,'es_menu_page'));
	...
	function es_inc_scripts() {
		wp_enqueue_style('easy-start',plugins_url().'/easy-start/css/easy-start.css');
	}	
	...
	function es_admin_scripts() {
		...
		wp_enqueue_style('es-admin',plugins_url().'/easy-start/css/es-admin.css');
		wp_enqueue_style('easy-start',plugins_url().'/easy-start/css/easy-start.css');

		
		

		wp_enqueue_script( 'easy-start-admin', plugins_url().'/easy-start/js/easy-start-admin.js', array( 'jquery','jquery-ui-core' ), false, true);
	}
